...
for p in $projects; do
  ...
  curl -fSs -X GET "$TX_API3/projects/o%3Ahisp-uio%3Ap%3A${p//\"/}" -H "Content: application/json" -H "Authorization: Bearer $TXTOKEN" | jq '.[].attributes' >/tmp/proj$$
  ...
  if [[ $tags == *"$MERGE_FLAG"* ]]; then
    ...
    branches=$(curl -fSs -X GET "$TX_API3/resources?filter%5Bproject%5D=o%3Ahisp-uio%3Ap%3A${p//\"/}" -H "Content: application/json" -H "Authorization: Bearer $TXTOKEN" | jq '.data[].attributes.slug | split("--")[0] | split("-") | join(".")' | uniq)
